...
const getRandomImage = () => {
  const imageList = [
    images.gaming1, images.gaming2, images.movie1, images.movie2, images.music1, images.music2
...
const Indoor = () => {
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      try {
        setLoading(true);
        const url = `${ngrokBaseUrl}/questions/category/indoors`;
  ...
  const saveTestResults = async () => {
    try {
      ...
      if (response.ok) {
        setDominantGroup(data.finalIndoorActivity); 
  ...
  return (
    <SafeAreaView className="flex-1 bg-icol p-6 justify-center items-center">
    <View style={{
        flex: 1,
        ...
        <>
          ...
          {question.options.map((option, index) => (
            <View key={index} className="mb-5">
              ...
              >
                <View
                  className={`w-6 h-6 rounded-full border-2 border-black ${selectedOption === option ? 'bg-icol2' : 'bg-transparent'} absolute left-[-50px]`}
                >
          ...
          <TouchableOpacity
            ...
            className="border-2 border-purple-500 rounded-lg bg-icol2 p-4 mt-5"
          >
