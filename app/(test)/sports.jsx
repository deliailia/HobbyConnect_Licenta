...
const Sports = () => {
  ...
  useEffect(() => {
    ...
    const fetchQuestion = async () => {
      try {
        ...
        setLoading(true);
        const url = `${ngrokBaseUrl}/questions/category/sports`;
  ...
  const saveTestResults = async () => {
    try {
      ...
      if (response.ok) {
        setDominantGroup(data.finalSport);
  ...
  return (
    ...
    <SafeAreaView className="flex-1 bg-scol p-6 justify-center items-center">
        ...
        <>
          ...
          {question.options.map((option, index) => (
            <View key={index} className="mb-5">
              ...
              >
                <View
                  className={`w-6 h-6 rounded-full border-2 border-black ${selectedOption === option ? 'bg-scol2' : 'bg-transparent'} absolute left-[-50px]`}
                >
          ...
          <TouchableOpacity
            ...
            className="border-2 border-green-500 rounded-lg bg-scol2 p-4 mt-5"
          >
