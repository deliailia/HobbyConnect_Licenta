...
const getRandomImage = () => {
  const imageList = [
    images.travelling1, images.travelling2, images.walking1, images.walking2, images.coffee1, images.coffee2,
...
const Outdoor = () => {
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      try {
        setLoading(true);
        const url = `${ngrokBaseUrl}/questions/category/outdoors`;
  ...
  const saveTestResults = async () => {
    try {
      ...
      if (response.ok) {
        setDominantGroup(data.finalOutdoorActivity); 
  ...
  return (
    ...
    <SafeAreaView className="flex-1 bg-ocol p-6 justify-center items-center">
        ...
        <>
          ...
          {question.options.map((option, index) => (
            <View key={index} className="mb-5">
              ...
              >
                <View
                  className={`w-6 h-6 rounded-full border-2 border-black ${selectedOption === option ? 'bg-ocol2' : 'bg-transparent'} absolute left-[-50px]`}
                >
          ...
          <TouchableOpacity
            ...
            className="border-2 border-blue-500 rounded-lg bg-ocol2 p-4 mt-5"
          >
