...
const Spanish = () => {
  ...
  const subcategoryName = 'spanish'; 
  const categoryName = 'Language';
  ...
  const checkMembership = async () => {
    ...
    if (!userId) {
      ...
      if (response.status === 200) {
        ...
        console.log(`Is user a member? ${memberExists}`);
        setIsMember(memberExists);
      } else {
        console.error('Error checking membership:', data.message);
      }
    } catch (error) {
    ...
    if (isMember) {
      setTimeout(() => {
        router.push({
          ...
          params: { groupName: 'Language', subcategoryName: 'spanish' },
        });
      }, 2000);
    }
  }, [isMember]);
  ...
  const sendRequestToAdmin = async () => {
    try {
      ...
      // Construiește URL-ul pentru cerere
      ...
      //console.log('Response status:', response.status); // Log pentru debugging
  ...
  return (
    ...
    <View
      style={{
        flex: 1,
        backgroundColor: Colors.languages.spanish,
    ...
    >
      ...
      >
        ...
        <Text
          ...
          numberOfLines={25}
        >
       This group is for anyone passionate about learning Spanish. Whether you are a beginner or looking to improve your fluency, join us for grammar tips, vocabulary practice, and meaningful discussions about the Spanish language and culture.
        </Text>
