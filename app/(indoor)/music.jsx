...
const Music = () => {
  ...
  const subcategoryName = 'music'; 
  const categoryName = 'Indoor';
    ...
    try {
      ...
      if (response.status === 200) {
        if (data.members && Array.isArray(data.members)) {
          const memberExists = data.members.some((member) => member.userReqId === userId); // Compară username
          console.log(`Is user a member? ${memberExists}`);
          setIsMember(memberExists);
        } else {
          console.error("No members found in data");
        }
      } else {
  ...
  return (
    <View
      style={{
        flex: 1,
        backgroundColor: Colors.indoor.music,
    ...
    >
      ...
      >
        ...
        <Text
          ...
          numberOfLines={25}
        >
          Whether you're a musician, singer, or just love listening — this is your space to share tracks, discover new sounds, and talk about everything music. From classical to hip-hop, all genres and vibes welcome.
        </Text>
        </ScrollView>
